...
class ConfigManagerV2_5:
    ...
    def _load_config(self):
        ...
        try:
            ...
            # Parse into Pydantic model with proper nested conversion
        ...
        try:
            ...
            if 'visualization_settings' in config_data:
                ...
                if isinstance(vis_settings, dict) and 'dashboard' in vis_settings:
                    dashboard_config = vis_settings['dashboard']
                    if isinstance(dashboard_config, dict) and 'modes_detail_config' in dashboard_config:
                        modes_config = dashboard_config['modes_detail_config']
                        if isinstance(modes_config, dict):
                            # Convert to DashboardModeCollection after model validation
                            from data_models.core_system_config import DashboardModeCollection
                            dashboard_config['modes_detail_config'] = DashboardModeCollection.model_validate(modes_config)
